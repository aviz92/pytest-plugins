...
class TestGetTestPathWithoutParameters:
    def test_simple_test(self) -> None:
        ...
        assert get_test_path_without_parameters(item) == "tests/test_foo.py::test_bar", (
            "Expected path without brackets"
        )
...
class TestGetTestNameWithoutParameters:
    def test_simple_test(self) -> None:
        item = make_item("tests/test_foo.py::test_bar")
        assert get_test_name_without_parameters(item) == "test_bar", (
            "Expected only the test function name"
        )

    def test_parametrized_test_strips_brackets(self) -> None:
        item = make_item("tests/test_foo.py::test_bar[param1]")
        assert get_test_name_without_parameters(item) == "test_bar", (
            "Expected test name without parameter suffix"
        )

    def test_class_method(self) -> None:
        item = make_item("tests/test_foo.py::TestClass::test_method")
        assert get_test_name_without_parameters(item) == "TestClass::test_method", (
            "Expected class::method format"
        )
...
class TestGetPytestTestName:
    ...
    def test_parametrized_test_includes_brackets(self) -> None:
        ...
        assert get_pytest_test_name(item) == "test_bar[x=1]", (
            "Expected test name with parameter brackets"
        )

    def test_class_method(self) -> None:
        item = make_item("tests/test_foo.py::TestClass::test_method")
        assert get_pytest_test_name(item) == "TestClass::test_method", (
            "Expected class::method format"
        )
...
class TestGetTestFullName:
    ...
    def test_parametrized_test_includes_param_dict(self) -> None:
        ...
        assert result == "test_bar[{'x': 1, 'y': 2}]", (
            f"Expected dict representation of params, got {result}"
        )
...
class TestGetTestFullPath:
    ...
    def test_parametrized_test_includes_param_dict(self) -> None:
        ...
        assert result == "tests/test_foo.py::test_bar[{'x': 1}]", (
            f"Expected full path with param dict, got {result}"
        )

    def test_class_method_without_params(self) -> None:
        item = make_item("tests/test_foo.py::TestClass::test_method")
        assert get_test_full_path(item) == "tests/test_foo.py::TestClass::test_method", (
